    ...
    try:
        while keeprunning:
            ...
            while '\n' in buffer:
                ...
                #print(f"SEND_STOP_DATA_LOGGING:serial_listen_thread: Received: {repr(rx_data)}")
                if rx_data == "222":
                    ...
                    serial_conn.write(("2,0\n").encode()) # keep sending stop
                    #print(f"SEND_STOP:serial_listen_thread: Sent: 1,0")
            
            if time.time() - start_time >= timeout:
                print("SEND_STOP_DATALOGGING:Timeout reached, shutting down.")
                serial_conn.close()
                stop_event.set()
...
def main():
    ...
    try:
        ...
        print(f"-----------------------SEND_STOP_DATALOGGING: main {e}-------------------------------------")
