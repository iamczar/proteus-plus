    ...
    try:
        while keeprunning:
            ...
            while '\n' in buffer:
                ...
                print(f"SEND_START_DATALOGGING:serial_listen_thread: Received: {repr(rx_data)}")
                if rx_data == "333":
                    ...
                    serial_conn.write(("3,0\n").encode()) # keep sending stop
                    #print(f"SEND_STOP:serial_listen_thread: Sent: 1,0")
                    
            # Check for timeout
            if time.time() - start_time >= timeout:
                print("SEND_START_DATALOGGING:serial_listen_thread: Timeout reached, shutting down.")
                serial_conn.close()
                stop_event.set()
...
def main():
    ...
    try:
        print(f"------------------------------SEND_START_DATA_LOGGING-----------------------------------------")
        ...
        print(f"-----------------------SEND_START_DATA_LOGGING: main {e}-------------------------------------")
