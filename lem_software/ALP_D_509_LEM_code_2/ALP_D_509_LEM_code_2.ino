//ALP-TN-509
//branch from unified stable v02i6   11 Nov 2024
// inverted all valve inputs (line 1503+)
// ignore stepper pin allocations
...
const unsigned long PUMPBASEINTERVAL = 300;           // base interval for pump stepping (ms) this defined the dispense rate for an LEM
...
struct C1515_REPORT_POS {
    ...
    static const int COMMAND = 2;
    static const int STATEID = 3;
    static const int OXYMEASURED = 4;
    static const int PRESSUREMEASURED = 5;
    static const int FLOWMEASURED = 6;
    static const int TEMPMEASURED = 7;
    static const int CIRCPUMPSPEED = 8;
    static const int PRESSUREPUMPSPEED = 9;
    static const int PRESSUREPID =10;
    static const int PRESSURESETPOINT =11;
    static const int PRESSUREKP =12;
    static const int PRESSUREKI =13;
    static const int PRESSUREKD =14;
    static const int OXYGENPID = 15;
    static const int OXYGENSETPOINT = 16;
...
case C1002_STATE_POS::PUMP1DIRPIN:      break;
case C1002_STATE_POS::PUMP1STEPPIN:       break;
case C1002_STATE_POS::PUMP2DIRPIN:     break;
case C1002_STATE_POS::PUMP2STEPPIN:    break;
case C1002_STATE_POS::PUMP3DIRPIN:       break;
case C1002_STATE_POS::PUMP3STEPPIN:        break;
case C1002_STATE_POS::PUMP4DIRPIN:     break;
case C1002_STATE_POS::PUMP4STEPPIN:        break;
...
void realtime(){
    //dispenseControl();
    if (timings.currentTime-circPump.lastStep>circPump.stepInterval && state.dispensePara==0){
        pumpStep(1);
        circPump.lastStep=circPump.lastStep+circPump.stepInterval;
        }

    if (timings.currentTime-pressurePump.lastStep>pressurePump.stepInterval && state.dispensePara==0){
        pumpStep(2);
        pressurePump.lastStep=pressurePump.lastStep+pressurePump.stepInterval;
        }

    if (timings.currentTime-reagentPump1.lastStep>reagentPump1.stepInterval && state.dispensePara==1){
        pumpStep(1);
        reagentPump1.lastStep=reagentPump1.lastStep+reagentPump1.stepInterval;
        }
    
    if (timings.currentTime-reagentPump2.lastStep>reagentPump2.stepInterval && state.dispensePara==2){
        pumpStep(2);
        reagentPump2.lastStep=reagentPump2.lastStep+reagentPump2.stepInterval;  
        }

    if (timings.currentTime-reagentPump3.lastStep>reagentPump3.stepInterval && state.dispensePara==3){
        pumpStep(3);
        reagentPump3.lastStep=reagentPump3.lastStep+reagentPump3.stepInterval;
        }

    if (timings.currentTime-reagentPump4.lastStep>reagentPump4.stepInterval && state.dispensePara==4){
        pumpStep(4);
...
if (state.dispensePara && !communicationData.stateRequestSent){
    if (measurementData.dispensedVolume>state.dispenseVolumeSP){
        state.dispenseVolumeSP=0;
        measurementData.dispensedVolume=0;
...
void setValves(){
    digitalWrite(state.valve1Pin, (1-state.valve1));
    digitalWrite(state.valve2Pin, (1-state.valve2));
    digitalWrite(state.valve3Pin, (1-state.valve3));
    digitalWrite(state.valve4Pin, (1-state.valve4));
    digitalWrite(state.valve5Pin, (1-state.valve5));
    digitalWrite(state.valve6Pin, (1-state.valve6));
    digitalWrite(state.valve7Pin, (1-state.valve7));
    digitalWrite(state.valve8Pin, (1-state.valve8));
    digitalWrite(state.valve9Pin, (1-state.valve9));
    digitalWrite(state.valve10Pin, (1-state.valve10));
    digitalWrite(state.valve11Pin, (1-state.valve11));
    digitalWrite(state.valve12Pin, (1-state.valve12));
    digitalWrite(state.valve13Pin, (1-state.valve13));
    digitalWrite(state.valve14Pin, (1-state.valve14));
    digitalWrite(state.valve15Pin, (1-state.valve15));
    digitalWrite(state.valve16Pin, (1-state.valve16));
}
